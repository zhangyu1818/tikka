...
const transform: Transform<TransformOptions> =
  ...
  async (state) => {
    ...
    const { cwd, source, readFile, outputFile, logger, remove, outDir: rootDir } = state
    ...
    for (const outDir of outDirs) {
      try {
        ...
        await outputFile(tscNeedPath, JSON.stringify(configOptions), false)
        ...
        if (originalContent) {
          await outputFile(tscNeedPath, originalContent, false)
        } else {
